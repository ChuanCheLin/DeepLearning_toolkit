...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_type', type=str, default= 'all', help='subsets need to be deleted')
    # parser.add_argument('--ext', type=str, default='.png', help='VOC图像数据后缀，注意带"." ' )
    
    opt = parser.parse_args()
    if opt.img_dir != "/home/eric/mmdetection/data/VOCdevkit/datasets/VOC2007/JPEGImages/":
        duplicate(all_jpg_path = "/home/eric/mmdetection/data/VOCdevkit/datasets/VOC2007/JPEGImages/", 
        output_jpg_path = opt.img_dir, ann_path = opt.anno_dir)
    ...
    if opt.img_dir != "/home/eric/mmdetection/data/VOCdevkit/datasets/VOC2007/JPEGImages/":
        shutil.rmtree(opt.img_dir)

    if opt.dataset_type == 'all':
        pass
    elif opt.dataset_type == 'few' or opt.dataset_type == 'base' or opt.dataset_type == 'balanced':
        shutil.rmtree(opt.coco_dir + 'train')
        shutil.rmtree(opt.coco_dir + 'val')
        shutil.rmtree(opt.coco_dir + 'test')    
        shutil.rmtree(opt.coco_dir + 'trainval')    
    elif opt.dataset_type == 'comparison':
        shutil.rmtree(opt.coco_dir + 'train')
        shutil.rmtree(opt.coco_dir + 'val')
        shutil.rmtree(opt.coco_dir + 'trainvaltest')    
